...
type
  {
   this defines a type which is an array of integers, where the subrange [1..24]
   defines the indices of the array, so they don't have to be zero-based like
   most other languages, but can be
  }
...
begin
  Total := 0;
  // using Low and High sets the boundaries of the array (1 to 24 in this case)
  // it's now independent of length so if the type was changed to [0..23] then
